...
public class LightSensorPoller {
	...
	//left and right colorID mode
	private SampleProvider lColorID;
	private SampleProvider rColorID;
	private SampleProvider lAvg;
	private SampleProvider rAvg;
	private float[] lColorIDData;
	private float[] rColorIDData;
	
	// Constructor
	/**
	 * Constructs a LightSensorPoller, initializes the sample providers, and
	...
	public LightSensorPoller() {
		...
		this.rColorIDData = new float[rColorID.sampleSize()];
	}
	
	public int getLColorID() { 
		lColorID.fetchSample(lColorIDData, 0); 
		int color = (int)lColorIDData[0]; 
		return color; 
	}
	public int getRColorID() { 
		rColorID.fetchSample(rColorIDData, 0); 
		int color = (int)rColorIDData[0]; 
		return color; 
	}
	 ...
	 */
	/*
	 * public int getFColorID() { fColorID.fetchSample(fColorIDData, 0); int
	 * color = (int)fColorIDData[0]; return color; }
	 */
	 ...
	 */
	/*
	 * public float[] getFColorRGB() { float[] tempRGB = new float[3];
	 * fColorRGB.fetchSample(fColorData, 0); tempRGB[0] = fColorData[0];
	 * tempRGB[1] = fColorData[1]; tempRGB[2] = fColorData[2]; return tempRGB; }
	 */
